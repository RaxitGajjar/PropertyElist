...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (cookieVal !== "true" && localVal !== "true") {
      ...
      // Async ટાઇમઆઉટ દ્વારા સેટસ્ટેટ કરવાથી રીએક્ટની કાસ્કેડિંગ રેન્ડર એરર કાયમ માટે દૂર થઈ જશે
      setTimeout(() => {
    ...
    <div className="p-8 font-sans text-slate-900 w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">🌐 Total Visitors (Traffic)</p>
          <p className="text-3xl font-black text-blue-600 mt-2">1,245</p>
          <span className="text-xs font-bold text-emerald-600 mt-1 inline-block">Today: +184</span>
        </div>

        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">📩 Total Leads Received</p>
          <p className="text-3xl font-black text-amber-500 mt-2">2</p>
        </div>

        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">🏢 Active Properties</p>
          <p className="text-3xl font-black text-emerald-600 mt-2">1</p>
        </div>

        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">📍 Top Location</p>
          <p className="text-2xl font-black text-purple-600 mt-2">Ahmedabad</p>
        </div>
      </div>

      {/* 📊 NEW REAL-TIME ANALYTICS GRAPH / VISUALIZER */}
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-base font-bold text-slate-800">📈 Weekly Traffic & Lead Generation Trend</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Traffic Source */}
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <h2 className="text-base font-bold text-slate-800 mb-4">🌐 Traffic Source (ક્યાંથી લોકો આવ્યા?)</h2>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <h2 className="text-base font-bold text-slate-800 mb-4">📍 Top Visitors by City (કયા સિટીમાંથી?)</h2>
          <div className="space-y-3">
            <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 border border-slate-100">
              <span className="font-bold text-slate-800 text-sm">📍 Ahmedabad</span>
              <span className="text-xs font-extrabold bg-blue-100 text-blue-800 px-3 py-1 rounded-full">820 Visitors</span>
            </div>
            <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 border border-slate-100">
              <span className="font-bold text-slate-800 text-sm">📍 Surat</span>
              <span className="text-xs font-extrabold bg-blue-100 text-blue-800 px-3 py-1 rounded-full">210 Visitors</span>
            </div>
            <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 border border-slate-100">
              <span className="font-bold text-slate-800 text-sm">📍 Vadodara</span>
              <span className="text-xs font-extrabold bg-blue-100 text-blue-800 px-3 py-1 rounded-full">115 Visitors</span>
            </div>
          </div>
        </div>
      </div>
